...
var HRS = {
  ...
  log:(t)=>{
    ...
    if(!acc.process_run() && set.def.slm) acc.check(80);
    if(t) {
      print("***start hrm log");
      this.hrmlog=setInterval(()=>{
        if(!this.process) HRS.start(30);
      },(t*60*1000)); // t minute
    }
  },
  bpm:[],
  start:(t)=>{
    ...
    if (this.process) {
      ...
      this.process=0;
      HRS.disable();
      if(acc.process_run() && !set.def.slm && (Date().getHours()>=valdef.sleeptime[0] || Date().getHours()<valdef.sleeptime[2])) acc.check(0);
    }
    HRS.on("hrmlog",hrmtocsv);
    HRS.enable();
    if(!acc.process_run()) acc.check(80); //enable accel at 12.5Hz
    acc.movehrm=0;
    HRS.bpm=[];
    var bpmtime=0;
    var beatcount=0;
